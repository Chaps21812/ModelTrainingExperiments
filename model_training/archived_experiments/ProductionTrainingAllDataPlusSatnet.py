...
if __name__ == "__main__":
    ...
    SATNET = "/data/Dataset_Compilation_and_Statistics/Sentinel_Datasets/SatNet1.4"
    
    ALL_TELESCOPES = [TELESCOPE_A,TELESCOPE_B,TELESCOPE_C,TELESCOPE_E, SATNET]
    ...
    for DL in data_limit:
        train_params = {
            ...
            "experiment_name": f"Production-NoRescale_Training",
            "epochs": 250,
            "batch_size": 30,
        ...
        for i, data_dir in enumerate(train_params["main_directories"]):
            if "SatNet" in os.path.basename(data_dir):
                sub_directories_train[i].append(os.path.join(data_dir, "train"))
                sub_directories_val[i].append(os.path.join(data_dir, "val"))
                continue
