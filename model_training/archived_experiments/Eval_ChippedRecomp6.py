...
def evaluate_single_dataset(model, dataloader:DataLoader, device:str, plot:bool=False) -> dict:
    ...
    pr_curves = calculate_pr_curves(total_preds, total_tgts)
    ...
    for tc in confidence_thresholds:
        for tf in fit_thresholds:
            results = centroid_l2_accuracy(total_preds, total_tgts, tconfidence=tc, tfit=tf)
...
def evaluate_on_test_sets(datasets:Union[str,list], model, output_directory:str, experiment_title, evaluation_metrics, device_no:torch.device, database=None):
    ...
    validation_loader = DataLoader(validation_set, batch_size=18, shuffle=True, collate_fn=lambda x: (zip(*x)))
...
if __name__ == "__main__":
    ...
    L1_R2 = "/data/Dataset_Compilation_and_Statistics/Sentinel_Datasets/Finalized_datasets/L1_10_overlap_R=2/models/NoStop_L1_O10_R2_2025-09-05_05:36/NoStop_L1_O10_R2_weights_E75.pt"
    L1_R5 = "/data/Dataset_Compilation_and_Statistics/Sentinel_Datasets/Finalized_datasets/L1_10_overlap_R=5/models/NoStop_L1_O10_R5_2025-09-05_05:36/NoStop_L1_O10_R5_weights_E75.pt"
    ...
    evaluation_params = {
        ...
        "run_name": f"Recomp_O=RDL{datetime.now().strftime('%Y-%m-%d_%H:%M')}",
        "model": L1_SMallRandom_Overlap_model,
        "dataset": L1_dataset,
        "device_no": 4,
