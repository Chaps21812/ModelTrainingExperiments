...
from evaluation.evaluation_metrics import centroid_accuracy, calculate_bbox_metrics, calculate_centroid_difference, centroid_l2_accuracy, calculate_pr_curves
...
def evaluate_over_time(model, dataset_directory:str, epoch:int, dataloader:DataLoader, evaluation_metrics:list, device:str, plot:bool=False):
    ...
    for metric in evaluation_metrics:
        results = metric(total_preds, total_tgts)
...
def perform_timewise_benchmark(datasets:list, model_path:str, output_directory:str, experiment_title, evaluation_metrics, device_no=5, database=None, batchsize=10):
    ...
    for index, path in tqdm(enumerate(database)):
        ...
        del results["PR_Curve_Precision"]
        del results["PR_Curve_Recall"]
        del results["PR_Curve_F1"]
        del results["PR_Curve_Fit"]
        del results["PR_Curve_Confidence"]
        del results["date"]
...
if __name__ == "__main__":
    ...
    dataset = ["/data/Dataset_Compilation_and_Statistics/Sentinel_Datasets/Finalized_datasets/Curriculum_L1_Low_SNR_TTS/val"]
    device_no = 5
    batchsize = 120
    #Adjust this code only

    metrics = [centroid_l2_accuracy, calculate_bbox_metrics, calculate_centroid_difference, calculate_pr_curves]
